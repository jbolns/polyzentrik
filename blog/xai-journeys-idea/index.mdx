...
There is much to like in the literature about explainable AI (XAI), which closely relates but is not exactly the same as interpretable AI.

But there seems to be, ironically, a need for better explanation and planning of the process involved in delivering on the promise of explainability and interpretability.
...
* **AI explainability** is driven by an interest in general but meaningful explanations. For simplicity and because this is a brainstorming post, one can think about it in terms of summaries – overview of causes and outcomes.
...
Explaining why something happened by way of a summary is a fundamentally different task from translating in detail why something happened. Yet, the terms are regularly conflated. Why? Why would we forget so easily about this subtle yet significant difference?
...
We believe this can be achieved with a risk pyramid containing the different levels of (un)explainability/interpretability an AI model can have. It then becomes possible to draw the journey by which different techniques make AI models more explainable or interpretable. 
...
* **STEP 1. This can give you an after-the-fact (ex-post) overview of why outcomes were as such, but the insight may not be complete due to documented shortcomings in SHAP.
...
* **STEP 3. Use the results of 1 & 2 to calibrate future models.** This might give you a degree of foresight into future outcomes. You might also be able to include some flags that enhance insight into crucial aspects of the AI process, thereby enabling some minor interpretability.

This can then be communicated more stylistically by including only the relevant steps.
...
Nothing ever works as first imagined, but we reckon there is something there worth pursuing further.

For this reason, we have made the pyramid above the basis for a new digital ethics project aimed at developing a conceptual module to facilitate the planning and communication of explainable/interpretable AI efforts. Read more about this project [here](http://localhost:8000/resources/xai-journeys/).

In theory, the risk pyramid and procedure outlined above can be used to explain any explainability or interpretability journey. We want to test that theory through additional brainstorming and real-world cases.
...
### Get in touch!
So, get in touch if you think any of the above makes any sense. We believe it does.
